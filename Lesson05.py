...
while answer != 'q':
	...
	if answer == 'Y':
		...
		if do == 1:
			...
			if ending != '':
				...
				if choice == 1:
					...
					if i != '':
						...
						print ('Скопирован ' + str(count) + ' файл.')
				elif choice == 2:
			...
			dirname = input ('Введите имя директории (в конце строки укажите две "\\"): ')
			ending = input ('Файлы с каким буквенным окончанием вы хотите удалить? ')
			if ending != '':
				count = remove_file(dirname)
				print ('Удалено ' + str(count) + ' файлов.')
			else:
